...
class postImageMainController extends Controller
{
    ...
    public function index(): View|Application|Factory|\Illuminate\Contracts\Foundation\Application|RedirectResponse
    {
        ...
        if ($redirect = $this->getters->getAdminAccess(type: 'access', key: 'view/post_content_main')) return $redirect;
        $this->getters->setSEOTitle('post_images_main');
        $data['heading_title'] = __('admin/page_titles.post_images_main');
        ...
        foreach ($data['data'] as $item) {
            $data['items'][] = ['id' => $item['id'], 'file' => $item['file'], 'link_delete' => route('content_main.destroy', $item['id'])];
        }
    ...
    {
        return redirect()->route('content_main.index');
    }
    ...
    {
        if (request()->ajax()) {
            if ($this->getters->getAdminAccess(type: 'modify', key: 'edit/post_content_main_delete')) return response()->json(['success' => true, 'message' => 'Доступ запрещен']);
        } else {
            if ($redirect = $this->getters->getAdminAccess(type: 'access', key: 'view/admin_panel', route: 'client')) return $redirect;
            if ($redirect = $this->getters->getAdminAccess(type: 'modify', key: 'edit/post_content_main_delete')) return $redirect;
        }
        ...
        if (request()->ajax()) {
            ...
            return redirect()->route('content_main.index')->with('success', __('admin/posts/post.notify_image_deleted'));
        }
    }
