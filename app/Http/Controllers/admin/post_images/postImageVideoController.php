...
class postImageVideoController extends Controller
{
    ...
    public function index(): View|Application|Factory|\Illuminate\Contracts\Foundation\Application|RedirectResponse
    {
        ...
        if ($redirect = $this->getters->getAdminAccess(type: 'access', key: 'view/post_content_video')) return $redirect;
        $this->getters->setSEOTitle('post_images_video');
        $data['heading_title'] = __('admin/page_titles.post_images_video');
        ...
        foreach ($data['data'] as $item) {
            $data['items'][] = ['id' => $item['id'], 'file' => $item['file'], 'link_delete' => route('content_video.destroy', $item['id'])];
        }

        $data['elements'] = $this->getters->getAdminHeaderFooter();

        return view('admin/post_images/video', ['data' => $data]);
    }

    public function create(): RedirectResponse
    {
        return redirect()->route('content_video.index');
    }
    ...
    {
        if (request()->ajax()) {
            if ($this->getters->getAdminAccess(type: 'modify', key: 'edit/post_content_video_delete')) return response()->json(['success' => true, 'message' => 'Доступ запрещен']);
        } else {
            if ($redirect = $this->getters->getAdminAccess(type: 'access', key: 'view/admin_panel', route: 'client')) return $redirect;
            if ($redirect = $this->getters->getAdminAccess(type: 'modify', key: 'edit/post_content_video_delete')) return $redirect;
        }
        ...
        if (request()->ajax()) {
            return response()->json(['success' => true, 'message' => __('admin/posts/post.notify_video_deleted')]);
        } else {
            return redirect()->route('content_video.index')->with('success', __('admin/posts/post.notify_video_deleted'));
        }
    }
