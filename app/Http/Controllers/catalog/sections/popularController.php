...
class popularController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->config = $this->getters->getSetting('page_popular') ?? ['count_per_page' => 20, 'watermark' => 0, 'template' => 'style_1'];
    }

    public function index()
    {
        $data['h1'] = __('catalog/page_titles.post_popular_h1');
        $data['title'] = __('catalog/page_titles.post_popular_t');
        $data['description'] = __('catalog/page_titles.post_popular_d');
        $this->getters->setMetaInfo(title: $data['title'], description: $data['description'], url: route('client.post.popular'));
        ...
        $breadcrumbs = [
            ['link' => route('client.post.popular'), 'title' => __('catalog/page_titles.post_popular')]
        ];
