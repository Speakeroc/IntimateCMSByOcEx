...
class individualController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->config = $this->getters->getSetting('page_individual') ?? ['count_per_page' => 20, 'watermark' => 0, 'template' => 'style_1'];
    }

    public function index()
    {
        $data['h1'] = __('catalog/page_titles.post_individual_h1');
        $data['title'] = __('catalog/page_titles.post_individual_t');
        $data['description'] = __('catalog/page_titles.post_individual_d');
        $this->getters->setMetaInfo(title: $data['title'], description: $data['description'], url: route('client.post.individual'));
        ...
        $data['data'] = $items = $d_v->union($v_v)->union($v)->paginate($this->config['count_per_page']);
        ...
        $breadcrumbs = [
            ['link' => route('client.post.individual'), 'title' => __('catalog/page_titles.post_individual')]
        ];
