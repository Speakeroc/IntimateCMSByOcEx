...
class allController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->config = $this->getters->getSetting('page_all') ?? ['count_per_page' => 20, 'watermark' => 0, 'template' => 'style_1'];
    }

    public function index()
    {
        $data['h1'] = __('catalog/page_titles.post_all_h1');
        $data['title'] = __('catalog/page_titles.post_all_t');
        $data['description'] = __('catalog/page_titles.post_all_d');
        $this->getters->setMetaInfo(title: $data['title'], description: $data['description'], url: route('client.post.all'));

        $template = $this->config['template'];
        $data['data'] = $items = Cache::remember('allController_query', 60, function () {
            ...
            return $d_v->union($d_n_v)->union($v_v)->union($v_n_v)->union($v)->union($n_v)->paginate(config('app.count_per_page'));
        });
        ...
        $breadcrumbs = [
            ['link' => route('client.post.all'), 'title' => __('catalog/page_titles.post_all')],
        ];
