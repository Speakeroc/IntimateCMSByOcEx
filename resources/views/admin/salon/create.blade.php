...
@section('content')
    <main id="main-container">
        <div class="bg-image" style="background-image: url('{{ url('images/admin/photo16@2x.jpg') }}');">
            <div class="content py-6">
                <h3 class="text-center text-white mb-0">
                    {{ __('admin/page_titles.salon_add') }}
                </h3>
            </div>
        </div>
        ...
        <div class="content">
            <form action="{{ route('salon.store') }}" method="post" id="form-save" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="random_id" id="input-random-id" value="{{ old('random_id') ?? rand(100000, 9999999) }}">
                <div class="block block-rounded">
                    <div class="block-header">
                        <h3 class="block-title">{{ __('admin/page_titles.salon_add') }}</h3>
                                        ...
                                        <div class="mb-2">
                                            ...
                                            <select class="form-select" id="select-user" name="user_id">
                                                @foreach($data['main_data']['users'] as $user)
                                                    <option value="{{ $user['id'] }}" @if((old('user_id') ?? $data['this_user_id']) == $user['id']) selected @endif>{{ __('admin/salon/salon.user_r', ['login' => $user['login'], 'email' => $user['email']]) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <!-- //Users -->
                    ...
                    <div class="block-content">
                        <div class="row">
                            <div class="col-sm-3 col-12 mb-2">
                                ...
                                @php $moderation_id = old('moderation_id') ?? null; @endphp
                                @php $moderation_text = old('moderation_text') ?? null; @endphp
