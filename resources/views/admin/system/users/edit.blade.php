        ...
        <div class="bg-image" style="background-image: url('{{ url('images/admin/photo16@2x.jpg') }}');">
            <div class="content py-6">
                <h3 class="text-center text-white mb-0">
                    {{ __('admin/page_titles.users_edit') }}
                </h3>
            </div>
        </div>

        <div class="content">
            <form action="{{ route('users.update', $data['id']) }}" method="post" id="form-save" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="block block-rounded">
                    <div class="block-header">
                        <h3 class="block-title">{{ __('admin/page_titles.users_edit') }}</h3>
                                ...
                                <div class="row">
                                    <div class="col-sm-6 col-12">
                                        <div class="mb-4">
                                            ...
                                            <input type="text" name="username" value="{{ old('username') ?? $data['username'] }}" class="form-control @error('username') is-invalid @enderror" id="username" placeholder="{{ __('admin/system/users.username') }}">
                                            @error('username')
                                            <script>
                                    ...
                                    <div class="col-sm-6 col-12">
                                        <div class="mb-4">
                                            ...
                                            <input type="text" name="login" value="{{ old('login') ?? $data['login'] }}" class="form-control @error('login') is-invalid @enderror" id="code" placeholder="{{ __('admin/system/users.login') }}">
                                            @error('login')
                                            <script>
                                                kbNotify('error', '{{ $message }}');
                                            </script>
                                    ...
                                    <div class="col-12">
                                        <div class="mb-4">
                                            ...
                                            <input type="text" name="email" value="{{ old('email') ?? $data['email'] }}" class="form-control @error('email') is-invalid @enderror" id="name" placeholder="{{ __('admin/system/users.email') }}">
                                            @error('email')
                                            <script>
                                                kbNotify('error', '{{ $message }}');
                                            </script>
                                    ...
                                    <div class="col-12">
                                        <div class="form-floating mb-4">
                                            <select class="form-select" id="select-email-activate" name="email_activate">
                                                <option value="1" @if((old('email_activate') ?? $data['email_activate']) == 1) selected @endif>{{ __('lang.yes') }}</option>
                                                <option value="0" @if((old('email_activate') ?? $data['email_activate']) == 0) selected @endif>{{ __('lang.no') }}</option>
                                            </select>
                                    ...
                                    <div class="col-12">
                                        <div class="mb-4">
                                            ...
                                            <div class="input-group">
                                                <input type="number" name="balance" value="{{ old('balance') ?? $data['balance'] }}" class="form-control @error('balance') is-invalid @enderror" id="input-balance" placeholder="{{ __('admin/system/users.balance') }}">
                                    ...
                                    <div class="col-12">
                                        <div class="mb-4">
                                            <label class="form-label" for="password">{{ __('admin/system/users.password') }} <small>({{ __('admin/system/users.password_help') }})</small></label>
                                ...
                                <div class="form-floating mb-4">
                                    <select class="form-select" id="select-user-group-id" name="user_group_id">
                                        @foreach($data['groups'] as $key)
                                            <option value="{{ $key['id'] }}" @if((old('user_group_id') ?? $data['user_group_id'])  == $key['id']) selected @endif>{{ $key['name'] }}</option>
                                        @endforeach
                                    </select>
    ...
    <script>
        ...
        buttons.forEach(button => {
            button.addEventListener('click', function() {
                const additionalBalance = parseFloat(this.getAttribute('data-balance'));
                const inputBalance = document.getElementById('input-balance');
                const currentBalance = parseFloat(inputBalance.value) || 0;
                inputBalance.value = currentBalance + additionalBalance;
            });
        });
    </script>
    <style>
        ...
        #user-preview-container img.preview-img {
            width: 296px;
            height: 296px;
            object-fit: contain;
        }
