        ...
        <div class="bg-image" style="background-image: url('{{ url('images/admin/photo16@2x.jpg') }}');">
            <div class="content py-6">
                <h3 class="text-center text-white mb-0">
                    {{ __('admin/page_titles.users_group_edit') }}
                </h3>
            </div>
        </div>

        <div class="content">
            <form action="{{ route('user_group.update', $data['id']) }}" method="post" id="form-save" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="block block-rounded">
                    <div class="block-header">
                        <h3 class="block-title">{{ __('admin/page_titles.users_group_edit') }}</h3>
                    ...
                    <div class="block-content">
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" value="{{ old('name') ?? $data['name'] }}" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="{{ __('admin/system/users_group.title') }}">
                            @error('name')
                            <script>
                                        ...
                                        @foreach($data['permissions']['access'] as $permission)
                                            ...
                                            @else
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" type="checkbox" id="switch-default{!! $permission['number'] !!}" name="permission[{{ $permission['type'] }}][]" value="{{ $permission['permission'] }}" {{ $permission['checked'] ? 'checked' : null }}>
