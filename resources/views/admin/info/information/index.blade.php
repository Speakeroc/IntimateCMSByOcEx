    ...
    <main id="main-container">
        <div class="bg-body-light">
            <div class="content content-full">
                <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                    <h1 class="flex-grow-1 fs-3 fw-semibold my-2 my-sm-3">{{ __('admin/page_titles.information') }}</h1>
                </div>
            </div>
        </div>
        ...
        <div class="content">
                ...
                <div class="block-content pt-0">
                    <form action="{{ route('information.index') }}" method="get">
                        <div class="row">
                            <div class="col-sm-3 col-12 my-2">
                                <div class="form-group">
                                    <label>{{ __('admin/info/information.title') }}</label>
                                    <input type="text" placeholder="{{ __('admin/info/information.title') }}" name="title" value="{{ $data['title'] }}" id="input-name" class="form-control">
                                </div>
                            </div>
                            <div class="col-sm-3 col-12 my-2">
                                <div class="form-group">
                                    <label>{{ __('admin/info/information.status') }}</label>
                            ...
                            <div class="col-sm-3 col-12 my-2">
                                <div class="form-group">
                                    <label>{{ __('admin/info/information.in_menu') }}</label>
                                    <select name="in_menu" id="input-in_menu" class="form-select">
                                        <option value="">{{ __('lang.no_select') }}</option>
                                        <option value="yes" @if($data['in_menu'] == "yes") selected @endif>{{ __('lang.status_on') }}</option>
                                        <option value="no" @if($data['in_menu'] == "no") selected @endif>{{ __('lang.status_off') }}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        <div class="d-flex align-items-center">
                            ...
                            @if($data['filtered'])
                                <a href="{{ route('information.index') }}" class="btn btn-danger">{{ __('lang.reset') }}</a>
                            @endif
                        </div>
                    </form>
                </div>
            ...
            <div class="block block-rounded">
                <div class="block-header">
                    <h3 class="block-title">{{ __('admin/info/information.list') }}</h3>
                    <div class="block-options">
                        ...
                        <a href="{{ route('information.create') }}" class="btn btn-sm btn-primary">{{ __('buttons.add') }}</a>
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-vcenter">
                            ...
                            <tr>
                                ...
                                <th>{{ __('admin/info/information.title') }}</th>
                                <th style="width: 15%;">{{ __('admin/info/information.info') }}</th>
                                <th class="text-center" style="width: 5%;">{{ __('admin/info/information.status') }}</th>
                                <th class="text-center" style="width: 15%;">{{ __('admin/info/information.created_at') }}</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-start gap-2">
                                            <div class="row">
                                                <div class="col-12">
                                                    <span class="badge bg-primary" data-bs-toggle="tooltip" title="{{ __('admin/info/information.views') }}"><i class="fa-solid fa-eye"></i> {{ $item['views'] }}</span>
                                                    <span class="badge {{ $item['in_menu'] ? 'bg-success' : 'bg-black-50' }}">{{ __('admin/info/information.in_menu') }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <form action="{{ route('information.destroy', $item['id']) }}" method="post">
                                            ...
                                            @method('DELETE')
                                            <div class="btn-group">
                                                <a href="{{ route('information.edit', $item['id']) }}" class="btn btn-alt-info">
    ...
    <script>
        $(document).ready(function () {
            ...
            deleteBtn.on('click', function () {
                ...
                $.ajax({
                    url: '{{ route("admin.information.massDelete") }}',
                    method: 'POST',
