...
@section('content')
    <main id="main-container">
        ...
        <div class="content">
            <form action="{{ route('news.store') }}" method="post" id="form-save" enctype="multipart/form-data">
                @csrf
                <div class="block block-rounded">
                    <div class="block-header">
                        <h3 class="block-title">{{ __('admin/page_titles.news_add') }}</h3>
                    ...
                    <div class="block-content">
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" class="form-control" id="input-title" placeholder="{{ __('admin/info/news.title') }}">
                        </div>
                        <div class="mb-4">
                            <label class="form-label" for="input-desc">{{ __('admin/info/news.desc') }}</label>
                            <textarea class="form-control desc" id="input-desc" name="desc" rows="4" placeholder="{{ __('admin/info/news.desc') }}">{{ old('desc') }}</textarea>
                        </div>
                        <div class="mb-4">
                            <label class="form-label" for="input-meta_title">{{ __('admin/info/news.meta_title') }}</label>
                            <input type="text" name="meta_title" value="{{ old('meta_title') }}" class="form-control" id="input-meta_title" placeholder="{{ __('admin/info/news.meta_title') }}">
                        </div>
                        <div class="mb-4">
                            <label class="form-label" for="input-meta-description">{{ __('admin/info/news.meta_description') }}</label>
                            <textarea class="form-control" id="input-meta-description" name="meta_description" rows="4" placeholder="{{ __('admin/info/news.meta_description') }}">{{ old('meta_description') }}</textarea>
                        </div>
                        <label class="form-label" for="example-text-input">{{ __('admin/info/news.banner') }}</label>
                        <div class="mb-4 position-relative">
                            <div id="upload-container" style="display: {{ old('image') ? 'none' : 'flex' }}">
                            ...
                            <div id="preview-container" style="display: {{ old('image') ? 'flex' : 'none' }};">
                                <img id="preview-image" src="{{ old('image') }}" alt="Preview" class="preview-img"/>
                        ...
                        <div class="row">
                            <div class="col-sm-4 col-12">
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="seo_url" value="{{ old('seo_url') }}" class="form-control" id="input-seo_url" placeholder="{{ __('admin/info/news.seo_url') }}">
                                </div>
                            </div>
                            <div class="col-sm-4 col-12">
                                <div class="mb-4">
                                    <label class="form-label" for="input-views">{{ __('admin/info/news.views') }}</label>
                                    <input type="number" name="views" value="{{ old('views') ?? 0 }}" class="form-control" id="input-views" placeholder="{{ __('admin/info/news.views') }}">
                                </div>
                            </div>
                            <div class="col-sm-4 col-12">
                                <div class="mb-4">
                                    <label class="form-label" for="input-like">{{ __('admin/info/news.like') }}</label>
                                    <input type="number" name="like" value="{{ old('like') ?? 0 }}" class="form-control" id="input-like" placeholder="{{ __('admin/info/news.like') }}">
                                </div>
                            </div>
                            <div class="col-sm-4 col-12">
                                <div class="mb-4">
                                    <label class="form-label" for="input-dislike">{{ __('admin/info/news.dislike') }}</label>
                                    <input type="number" name="dislike" value="{{ old('dislike') ?? 0 }}" class="form-control" id="input-dislike" placeholder="{{ __('admin/info/news.dislike') }}">
                                </div>
                            </div>
                            <div class="col-sm-4 col-12">
                                ...
                                <div class="space-x-2">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" id="radios-pinned-status-1" name="pinned" value="1" {{ (old('pinned') == 1) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="radios-pinned-status-1">{{ __('admin/info/news.pinned_yes') }}</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" id="radios-pinned-status-2" name="pinned" value="0" {{ (old('pinned') == 0) ? 'checked' : '' }}>
                            ...
                            <div class="col-sm-4 col-12">
                                <div class="mb-4">
                                    <div class="form-group">
                                        @php $created_at = (old('created_at') ? \Carbon\Carbon::parse(old('created_at'))->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i')); @endphp
