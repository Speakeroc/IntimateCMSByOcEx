...
@section('content')
    <main id="main-container">
        <div class="bg-image" style="background-image: url('{{ url('images/admin/photo16@2x.jpg') }}');">
            <div class="content py-6">
                <h3 class="text-center text-white mb-0">
                    {{ __('admin/page_titles.banner_add') }}
                </h3>
            </div>
        </div>
        ...
        <div class="content">
            <form action="{{ route('banner.store') }}" method="post" id="form-save" enctype="multipart/form-data">
                @csrf
                <div class="block block-rounded">
                    <div class="block-header">
                        <h3 class="block-title">{{ __('admin/page_titles.banner_add') }}</h3>
                                ...
                                <div class="row">
                                    <div class="col-sm-6 col-12">
                                        <div class="mb-4">
                                            ...
                                            <input type="text" name="title" value="{{ old('title') }}" class="form-control" id="input-title" placeholder="{{ __('admin/info/banner.title') }}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-12">
                                        <div class="mb-4">
                                            <label class="form-label" for="input-link">{{ __('admin/info/banner.link') }}</label>
                                            <input type="text" name="link" value="{{ old('link') }}" class="form-control" id="input-link" placeholder="{{ __('admin/info/banner.link') }}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-12">
                                        <div class="mb-4">
                                            <label class="form-label" for="input-sort_order">{{ __('admin/info/banner.sort_order') }}</label>
                                            <input type="text" name="sort_order" value="{{ old('sort_order') }}" class="form-control" id="input-sort_order" placeholder="{{ __('admin/info/banner.sort_order') }}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-12">
