        ...
        <div class="bg-image" style="background-image: url('{{ url('images/admin/photo16@2x.jpg') }}');">
            <div class="content py-6">
                <h3 class="text-center text-white mb-0">
                    {{ __('admin/page_titles.category_edit') }}
                </h3>
            </div>
        </div>

        <div class="content">
            <form action="{{ route('category.update', $data['id']) }}" method="post" id="form-save" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="block block-rounded">
                    <div class="block-header">
                        <h3 class="block-title">{{ __('admin/page_titles.category_edit') }}</h3>
                        ...
                        <div class="row">
                            <div class="col-sm-6 col-12">
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="title" value="{{ old('title') ?? $data['title'] }}" class="form-control @error('title') is-invalid @enderror" id="input-title-" placeholder="{{ __('admin/posts/category.title') }}">
                                    @error('title')
                                    <script>
                                ...
                                <div class="mb-4" style="user-select:none;">
                                    ...
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" value="0" id="only_verify_0" name="only_verify" {{ ((old('only_verify') ?? $data['only_verify']) == 0) ? "checked" : "" }} onclick="toggleCheckbox('only_verify_0', 'only_verify_1')">
                                        <label class="form-check-label" for="only_verify_0">{{ __('admin/posts/category.no_only_verify') }}</label>
                                    </div>
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" value="1" id="only_verify_1" name="only_verify" {{ ((old('only_verify') ?? $data['only_verify']) == 1) ? "checked" : "" }} onclick="toggleCheckbox('only_verify_1', 'only_verify_0')">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="meta_title" value="{{ old('meta_title') ?? $data['meta_title'] }}" class="form-control" id="input-meta_title" placeholder="{{ __('admin/posts/category.meta_title') }}">
                                </div>
                                <div class="mb-4">
                                    <label class="form-label" for="example-text-input">{{ __('admin/posts/category.meta_description') }}</label>
                                    <input type="text" name="meta_description" value="{{ old('meta_description') ?? $data['meta_description'] }}" class="form-control" id="input-meta_description" placeholder="{{ __('admin/posts/category.meta_description') }}">
                                </div>
                            ...
                            <div class="col-sm-6 col-12">
                                <div class="mb-4">
                                    ...
                                    <textarea name="description" class="ck_description">{{ old('description') ?? $data['description'] }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
