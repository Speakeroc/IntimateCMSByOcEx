...
@section('content')
    <main id="main-container">
        <div class="bg-image" style="background-image: url('{{ url('images/admin/photo16@2x.jpg') }}');">
            <div class="content py-6">
                <h3 class="text-center text-white mb-0">
                    {{ __('admin/page_titles.blacklist_edit') }}
                </h3>
            </div>
        </div>
        ...
        <div class="content">
            <form action="{{ route('blacklist.update', $data['id']) }}" method="post" id="form-save" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="block block-rounded">
                    <div class="block-header">
                        <h3 class="block-title">{{ __('admin/page_titles.blacklist_edit') }}</h3>
                        ...
                        <div class="row">
                            <div class="col-sm-6 col-12">
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="phone" value="{{ old('phone') ?? $data['phone'] }}" class="form-control" id="input-phone" placeholder="{{ __('admin/posts/blacklist.phone') }}">
                                    <script>
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea class="form-control" id="input-text" name="text" rows="4" placeholder="{{ __('admin/posts/blacklist.text') }}">{{ old('text') ?? $data['text'] }}</textarea>
                                </div>
                                <div class="mb-4">
                                    <div class="form-group">
                                        ...
                                        <select name="rating" id="input-rating" class="form-select">
                                            @for($i = 1; $i <= 5; $i++)
                                                <option value="{{ $i }}" @if((old('rating') ?? $data['rating']) == $i) selected @endif>{{ trans_choice('admin/posts/blacklist.rating_choice', $i, ['num' => $i]) }}</option>
                                            @endfor
                                        </select>
                                    </div>
                                </div>
