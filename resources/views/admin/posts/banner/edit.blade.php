...
@section('content')
    <main id="main-container">
        <div class="bg-image" style="background-image: url('{{ url('images/admin/photo16@2x.jpg') }}');">
            <div class="content py-6">
                <h3 class="text-center text-white mb-0">
                    {{ __('admin/page_titles.post_banner_edit') }}
                </h3>
            </div>
        </div>
        ...
        <div class="content">
            <form action="{{ route('banner_post.update', $data['id']) }}" method="post" id="form-save" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="block block-rounded">
                    <div class="block-header">
                        <h3 class="block-title">{{ __('admin/page_titles.post_banner_edit') }}</h3>
                            ...
                            <div class="col-sm-6 col-12">
                                <div class="row">
                                        ...
                                        <div class="mb-2">
                                            ...
                                            <select class="form-select" id="select-post-id" name="post_id">
                                                ...
                                                @foreach($data['posts'] as $post)
                                                    <option value="{{ $post['id'] }}" {{ ((old('post_id') ?? $data['post_id']) == $post['id']) ? 'selected' : '' }}>{{ $post['title'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="mb-2">
                                            <div class="form-group">
                                                @php $link = old('link') ?? ($data['link'] ?? null); @endphp
                                                <label class="form-label" for="input-activation-date">{{ __('admin/posts/banner.link') }}</label>
                                                <input type="text" class="form-control" id="input-activation-date" name="link" value="{{ $link }}" placeholder="{{ __('admin/posts/banner.link_p') }}">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-12">
                                        ...
                                        <div class="space-x-2">
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" id="radios-activation-1" name="activation" value="1" {{ ((old('activation') ?? $data['activation']) == 1) ? 'checked' : '' }}>
                                                <label class="form-check-label" for="radios-activation-1">{{ __('lang.status_on') }}</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" id="radios-activation-2" name="activation" value="0" {{ ((old('activation') ?? $data['activation']) == 0) ? 'checked' : '' }}>
                                    ...
                                    <div class="col-sm-6 col-12">
                                        <div class="mb-4">
                                            <div class="form-group">
                                                @php $activation_date = ((old('activation_date') ?? $data['activation_date']) ? \Carbon\Carbon::parse((old('activation_date') ?? $data['activation_date']))->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i')); @endphp
